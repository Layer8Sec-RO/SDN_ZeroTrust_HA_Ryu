# ha_zerotrust_snort.py - SDN Controller con Zero Trust, HA, monitoreo Snort y bloqueo reactivo (basado en IP destino)
from ryu.base import app_manager
from ryu.controller import ofp_event
from ryu.controller.handler import CONFIG_DISPATCHER, set_ev_cls
from ryu.ofproto import ofproto_v1_3
import requests, threading, time, json, ipaddress
from http.server import BaseHTTPRequestHandler, HTTPServer

# ---------- HTTP Server interno que recibe alertas desde Snort ----------
class SnortAlertHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        dst = data.get("dst_ip")
        if src and dst:
            UnifiedZeroTrustPolicy.registrar_alerta(src, dst)
            print(f"\n🔔 Alerta de Snort: {src} → {dst}  ⇒ bloqueo de subred")
...
class UnifiedZeroTrustPolicy(app_manager.RyuApp):
    ...
    bloqueos = {}  # { src_ip: set(subredes bloqueadas) }

    @classmethod
    def registrar_alerta(cls, src_ip, dst_ip):
        # Determinar subred del destino
        subredes = [
        ...
        for red in subredes:
            if dst in ipaddress.IPv4Network(red):
        ...
        while True:
            ...
            with self.lock:
                if best != self.best_server:
                    self.logger.info("[HA] Cambio de mejor servidor: %s (carga: %.1f)", best, self.get_load(best))
    ...
    def add_flow(self, dp, prio, match, actions):
        ...
        i = [p.OFPInstructionActions(o.OFPIT_APPLY_ACTIONS, actions)]
        dp.send_msg(p.OFPFlowMod(datapath=dp, priority=prio, match=match, instructions=i))

    def bloquear(self, dp, ip, red):
        p = dp.ofproto_parser
        m = p.OFPMatch(eth_type=0x0800, ipv4_src=ip, ipv4_dst=(red))
        self.add_flow(dp, 450, m, [])
    ...
    def flujos_http(self, dp):
        ...
        for ip in ["192.168.10.100", "192.168.20.100"]:
            ...
            match = p.OFPMatch(eth_type=0x0800, ip_proto=6, ipv4_src=ip, ipv4_dst=self.virtual_ip, tcp_dst=80)
            actions = [p.OFPActionSetField(ipv4_dst=self.best_server)]
            if ip == "192.168.10.100":
                actions += [p.OFPActionOutput(o.OFPP_NORMAL)]
            elif ip == "192.168.20.100":
                actions += [p.OFPActionOutput(4)]
    ...
    def delete_http(self, dp):
        ...
        for ip in ["192.168.10.100", "192.168.20.100"]:
            for m in [
                ...
                p.OFPMatch(eth_type=0x0800, ip_proto=6, ipv4_src=self.best_server, ipv4_dst=ip, tcp_src=80)
            ]:
